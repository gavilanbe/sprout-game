...
function bossHit(b,n){ if(b.hp>2){ b.hp=Math.max(2,b.hp-n); b.flash=10; SFX.ehit(); hitStop=Math.max(hitStop,4); flyText.push({x:b.x+b.w/2,y:b.y-4,txt:''+n,t:26,col:'#fffbe8'}); hitSpark(b.x+b.w/2,b.y+b.h/2); b.squash=.3;
...
function bossClang(b,cx,cy){ // la Hoja rebota: sin daño, con chispa y retroceso
  if(b.clangT>0) return; b.clangT=14; SFX.block(); shake=Math.max(shake,2);
...
function vientoBrazierLit(){ const b=boss; if(!b||b.type!=='viento') return; const [n,t]=vientoLit();
  showToast('BRASEROS '+n+'/'+t,n>=t?'¡el frío cede!':'el viento se debilita');
...
function vientoBlowOut(k){ const lit=[]; for(let y=0;y<SH;y++) for(let x=0;x<SW;x++) if(grid[y][x]===';') lit.push([x,y]);
  ...
  if(lit.length) { markDirty(); SFX.swoosh&&SFX.swoosh(); } }
function updViento(){
...
function hookBoss(D){
  ...
  for(let s=8;s<=120;s+=4){ const px=x0+D[0]*s, py=y0+D[1]*s; if(px>R[0]&&px<R[0]+R[2]&&py>R[1]&&py<R[1]+R[3]){
    ...
    b.vx=(tx-b.x)/14; b.vy=(ty-b.y)/14; bossRope={t:18}; SFX.sword(); noise(.1,.05,true); shake=4; return true; } }
  return false;
}
