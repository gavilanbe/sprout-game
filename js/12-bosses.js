...
function updViento(){
  ...
  if(b.st==='float'){
    ...
    if(b.t<=0){ b.st='aim'; b.t=enraged?30:40; }
  } else if(b.st==='aim'){
    b.row=player.y-8;
    if((tick&2)===0) parts.push({x:cx+(Math.random()-.5)*14,y:cy,vx:0,vy:0,life:10,col:'#fff',nog:true});
    if(b.t<=0){ const sv=enraged?4.4:3.6; b.st='sweep'; b.x=player.x<80?-32:160; b.vx=player.x<80?sv:-sv; b.y=b.row; SFX.ehit(); }
  } else if(b.st==='sweep'){
    b.x+=b.vx; if((tick&1)===0) parts.push({x:cx,y:cy,vx:-b.vx*.4,vy:(Math.random()-.5),life:12,col:'#cfe8ff',nog:true});
    if(b.x<-36||b.x>164){ b.st='rest'; b.t=100; b.x=Math.max(8,Math.min(120,player.x-8)); b.y=84; SFX.bump(); shake=6; }
...
function updAvispa(){
  ...
  else if(b.st==='hover'){ b.x+=Math.sin(tick*.06)*1.1; b.y=10+Math.sin(tick*.11)*6; b.x=Math.max(8,Math.min(120,b.x)); if(b.t<=0){ b.st='aim'; b.t=34; } }
...
function updTopo(){
  ...
  if(b.st==='yield'){ b.t=999; if((tick&7)===0) parts.push({x:b.x+8+Math.random()*16,y:b.y+24,vx:(Math.random()-.5)*.4,vy:-.3,life:10,col:'#8a7460',nog:true}); }
    ...
    if(b.t<=0){ b.st='up'; b.t=100; b.x=b.mx; b.y=b.my; SFX.edie(); shake=6; puff(b.x+16,b.y+24,'#5a4a40',12,1.6); } }
  else { const cx=b.x+16, cy=b.y+16;
    if(b.t===75||b.t===45){ const d=Math.hypot(player.x+8-cx,player.y+12-cy)||1;
...
function updMidboss(){
  ...
  if(m.type==='king'){ // patrulla, carga hacia ti; solo la cola es blanda
    ...
    else if(m.st==='charge'){ const [bx]=move(m.x+m.dir*2.4,m.y); if(bx||m.t<=0){ m.st='stuck'; m.t=60; shake=6; SFX.bump(); puff(cx+m.dir*10,cy,'#8a7460',8,1.2); } }
