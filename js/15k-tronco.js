...
function tkDroneAt(P,u){ const m0=P.st.m0; // dónde está el Zángano en el guion
  if(u<4) return null;
  if(u<48){ const k=presentSeg(u,4,46), e=presentEase.out(k); return [172+(m0.x-172)*e+Math.sin(k*9.42)*16*(1-k), m0.y-16*Math.sin(k*Math.PI)+Math.sin(u*.3)*2]; }
  if(u<54) return [m0.x,m0.y-8*presentEase.out(presentSeg(u,48,54))];                // coge impulso
  if(u<60) return [m0.x,m0.y-8+58*presentEase.in(presentSeg(u,54,60))];              // ¡picado!
  if(u<74) return [m0.x,m0.y+50*(1-presentEase.out(presentSeg(u,60,74)))];           // y sube como si nada
...
BOSS_INTRO.drone={ dur:178, shortDur:88,
  start(P){ const m=midboss; P.st.m0={x:m.x,y:m.y}; P.st.o=P.short?66:0; bossHidden=true; },
  tick(t,P){ const u=t+P.st.o, m0=P.st.m0;
    if(u>=4&&u<48&&u%12===4) tkBuzz(.02+.02*(u/48),.3,190);
    if(u===54) tkBuzz(.06,.2,260);
    if(u===60){ tkThud(true); shake=Math.max(shake,5); for(let i=0;i<12;i++){ const a=Math.PI+i/11*Math.PI; parts.push({k:'dust',x:m0.x+12+Math.cos(a)*7,y:m0.y+72,vx:Math.cos(a)*1.1,vy:-.2-Math.random()*.4,life:20,max:20,r:1+(i&1),col:groundDustCol(),nog:true}); }
      parts.push({x:m0.x+12,y:m0.y+70,vx:0,vy:0,life:10,col:'#fff8c0',ring:true,r:14,nog:true}); }
    if(u===78) tkThud(false);
    if(u===84){ tkThud(true); tkChord(67); shake=Math.max(shake,4); }
    if(u===112) tkShing();
  ...
  draw(t,P){ const u=t+P.st.o, rem=tkRem(P), pos=tkDroneAt(P,u);
    if(pos){ const x=Math.round(pos[0]), y=Math.round(pos[1]);
      ...
      if(u<48){ ctx.fillStyle='rgba(255,248,200,.55)'; for(let i=0;i<5;i++){ const yy=y+4+i*4, L=6+((i*7+u)%9); ctx.fillRect(x+26+(i&1)*3,yy,L,1); } }
...
BOSS_INTRO.avispa={ dur:330, shortDur:112,
  start(P){ const b=boss; P.st.b0={x:b.x,y:b.y}; P.st.o=P.short?160:0; bossHidden=true; if(!P.short) setTrack('silencio'); },
  ...
  draw(t,P){ const u=t+P.st.o, rem=tkRem(P), E=P.Q.echo, pal=E?TK_ECHO:TK_HONEY, cellPal=E?TK_ECHO.slice(1):TK_AMBER, b0=P.st.b0, cx=b0.x+16, qy=tkQueenY(P,u), cy=qy+16;
    ...
    if(u<=120){ const k=presentSeg(u,100,116); if(k>0){ ctx.globalAlpha=k; ctx.drawImage(E?tkQueenSilE():tkQueenSil(),b0.x,b0.y-10); ctx.globalAlpha=1; }
      if(u<118) for(let i=0;i<TK_QPTS.length;i++){ const [x,y]=tkQueenBee(i,u,P), f=(u+i)>>1; tkBee(x,y,f,Math.cos(i*2.39996+u*.075)<0?1:-1,pal); }
    ...
    if(u>=170){ const fade=Math.min(1,rem/14); ctx.globalAlpha=fade;
      ...
      const bk=presentEase.out(presentSeg(u,170,178)), bw=Math.round(VW*bk), bx0=Math.round(80-bw/2), y0=78, BH=44;
...
BOSS_OUTRO.avispa={ dur:220, swap:110, dest:TK_DEST,
  start(P){ P.st.p0=[player.x,player.y]; player.x=-400; },
  tick(t,P){ if(t<220) player.x=-400; // Sprout va dentro de la gota: lo pinta la salida
