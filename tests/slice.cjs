...
(async()=>{
  ...
  try{
    ...
    await check('Los tres atlas cargan; caminar y atacar tienen 16 celdas con alfa utilizable',async()=>{
      const stats=await ev(async()=>{const {loadAssets}=await import('/js/slice/assets.js');const a=await loadAssets();return {sizes:a.masters,walk:a.walk.length,attack:a.attack.length,transparent:[a.walk[0],a.attack[0]].map(c=>c.getContext('2d').getImageData(0,0,1,1).data[3]),smoothing:__slice.info().smoothing};});
      assert.equal(stats.walk,16);assert.equal(stats.attack,16);assert.deepEqual(stats.transparent,[0,0]);assert.equal(stats.smoothing,false);assert.equal(stats.sizes.hero[0],1254);
    });
    await check('Inicio por teclado, diálogo y cuatro direcciones',async()=>{
      ...
      const dirs=await ev(()=>{return ['ArrowDown','ArrowLeft','ArrowRight','ArrowUp'].map((key,i)=>{__slice.place(0,190,140);__slice.key(key);__slice.step(10);__slice.release(key);const s=__slice.info();return [s.player.dir,s.player.x-190,s.player.y-140];});});
      assert.deepEqual(dirs,[[0,0,12.5],[1,-12.5,0],[2,12.5,0],[3,0,-12.5]]);
    });
    await check('El agua bloquea el paso y el puente permite cruzar',async()=>{
      ...
      assert(positions[0]<=121);assert(positions[1]>190);
    });
    ...
    await check('Se vuelve andando por el puente abierto, se planta y la misión termina',async()=>{
      const s=await ev(()=>{
        ...
        const arrived=__slice.info().room;__slice.key('ArrowUp');__slice.step(48);__slice.release('ArrowUp');__slice.key('KeyZ');__slice.release('KeyZ');__slice.step(15);__slice.key('KeyZ');__slice.release('KeyZ');return {arrived,planted:__slice.info().planted,mode:__slice.info().mode};
      });assert.deepEqual(s,{arrived:0,planted:true,mode:'complete'});assert(await page.locator('#complete-screen').isVisible());
    });
